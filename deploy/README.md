...
Fill in real database, Redis, and SMTP values on the VM only. The email verification service sends the FYP-UKM R Mall Demo template through Brevo SMTP, so registration and forgot-password email codes require all three `BREVO_*` values. Do not commit real secrets.
