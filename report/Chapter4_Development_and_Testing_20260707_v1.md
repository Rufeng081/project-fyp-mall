...
| Table 4.3 | Repository and Artefact Structure |
...
| Layer | Technology | Evidence | Main role |
|---|---|---|---|
| Frontend | Vue 2.6.14, Vue Router 3.5.1, Vuex 3.6.2 | `ElectronicMallVue/package.json` | Customer and administrator SPA views. |
| HTTP client | Axios 0.26.1 | `ElectronicMallVue/package.json` | Centralised API calls, token header injection and response handling. |
| UI components | Element UI 2.15.6 | `ElectronicMallVue/package.json` | Forms, tables and management interface widgets. |
| Backend | Spring Boot 2.5.6, Java 8 | `ElectronicMallApi/pom.xml` | REST controllers, services, validation and executable packaging. |
| Persistence | MyBatis, MyBatis-Plus, MySQL | `ElectronicMallApi/pom.xml`, `database/electronic_mall.sql` | SQL mapping and durable data storage. |
| Temporary state | Redis | `RedisConstants.java`, service code | Login sessions, email codes, cooldown keys and selected cache state. |
| Email delivery | Spring Mail, Brevo SMTP | `EmailVerificationService.java`, deployment records | Verification-code delivery for registration and reset. |
| Deployment | Nginx, systemd, Google Cloud VM | `deploy/`, cloud records | Static frontend serving, reverse proxy and backend process management. |
| Performance testing | Apache JMeter 5.6.3 | `docs/testing/jmeter/`, Phase 6 records | Smoke, load, authenticated and mutation testing. |
...
### 4.2.4 Repository and Artefact Structure

The repository structure separates concerns between frontend, backend, database, deployment, testing and documentation. This structure made it possible to trace a Chapter 4 claim back to source evidence instead of relying on generic e-commerce descriptions.

**Table 4.3: Repository and Artefact Structure**

| Directory / file group | Implemented artefacts |
|---|---|
| `ElectronicMallVue/` | Vue pages, router, Vuex store, Axios utility, UI checks and production build configuration. |
| `ElectronicMallApi/` | Spring Boot controllers, services, DTOs, interceptors, entities, mappers, configuration and tests. |
| `database/` | Canonical MySQL schema and migration scripts. |
| `deploy/nginx/` | Nginx site configuration for Vue history fallback and API reverse proxy. |
| `deploy/systemd/` | systemd unit for running the Spring Boot API on the VM. |
| `deploy/env/` | Runtime environment template without secrets. |
| `docs/verification/` | Repeatable verification workflow. |
| `docs/testing/jmeter/` | JMeter plans and retained performance result summaries. |
| `docs/reports/` | Cloud deployment, email verification and JMeter phase reports. |
...
The MySQL database is named `electronic_mall` and is initialised through `database/electronic_mall.sql`. It stores users, products, variants, carts, order headers, order line items, categories, carousel records and uploaded-resource metadata.
...
The main evidence sources are `report/D6_Implementation_20260707_v3.md`, `docs/verification/verification-workflow.md`, `docs/reports/phase-4-cloud-deployment-report.md`, `docs/testing/jmeter/README.md`, `docs/records/phase-6-jmeter-execution-record-2026-06-16.md`, and `docs/reports/phase-6-jmeter-performance-evaluation-report.md`.
...
|---|---|
| Backend tests | `mvn -q test` exits with code 0 and email/authentication behaviours pass. |
...
| Reporting | Required testing evidence is generated, summarised and retained in the repository. |
...
|---|---|---|---|---|
| FR-01 | Email verification code | Run `mvn -q -Dtest=EmailVerificationServiceTest test` | Mocked Redis and mail sender | Pass if six-digit code storage, five-minute TTL, 60-second cooldown, wrong-code rejection, successful-code deletion and SMTP failure cleanup are verified. |
| FR-02 | Email registration and password reset | Run `mvn -q -Dtest=UserServiceEmailAuthTest test` | Email registration and reset DTOs | Pass if code verification, email normalisation, duplicate email rejection, user saving, password reset and token return succeed. |
| FR-03 | Full backend suite | Run `mvn -q test` | Backend test suite | Pass if all tests exit with code 0. |
| FR-04 | Backend package build | Run `mvn -q package` | Spring Boot Maven project | Pass if build succeeds and a deployable JAR is generated. |
| FR-05 | Frontend authentication wiring | Run `npm run check:auth` | Login, Register, request utility and router code | Pass if authentication-flow wiring is valid. |
| FR-06 | Frontend deployment configuration | Run `npm run check:deployment` | Production env, Axios, Nginx and systemd files | Pass if production API uses `/api` and browser code does not depend on `localhost:9191`. |
| FR-07 | Frontend history routes | Run Vue dev server and `npm run check:routes` | Main frontend routes | Pass if checked routes return the SPA entry HTML. |
| FR-08 | Frontend production build | Run `npm run build` | Vue source code | Pass if build exits with code 0. |
...
| FR-10 | Core API golden path | Run `node tools/phase12-api-golden-path.js` where services are available | Product, user, cart, order and simulated-payment data | Pass if product, login, cart, order, simulated payment and paid order history are verified. |
...
| SEC-02 | Expired-token cleanup | Check `request.js` through `npm run check:auth` | Pass if response code `401` or `402` clears local session and redirects to login. |
...
| ID | JMeter plan | Scenario | Threads | Pass / fail criteria |
|---|---|---|---:|---|
| JM-01 | `01_homepage.jmx` | Homepage and initial storefront APIs | Smoke: 1; load: 10, 50, 100, 200 | Pass if 0 JTL errors and assertion finds `id="app"`. |
| JM-02 | `02_product_list.jmx` | Product list browsing | Smoke: 1; load: 10, 50, 100, 200 | Pass if 0 JTL errors and JSON contains success code. |
| JM-03 | `03_product_detail.jmx` | Product detail and variants | Smoke: 1; load: 10, 50, 100, 200 | Pass if 0 JTL errors and JSON contains success code. |
| JM-04 | `04_login.jmx` | Demo user login | Smoke: 1; load: 10, 50, 100 | Pass if login succeeds and token can be extracted. |
| JM-05 | `08_order_history.jmx` | Authenticated order history | Smoke: 1; load: 10, 50, 100 | Pass if authenticated request succeeds with 0 JTL errors. |
| JM-06 | `05_add_to_cart.jmx` | Controlled add-to-cart mutation | Smoke: 1; mutation: 1, 5, 10 | Pass if 0 JTL errors and mutation remains controlled. |
| JM-07 | `06_place_order.jmx` | Controlled order placement | Smoke: 1; mutation: 1, 5, 10 | Pass if 0 JTL errors and order records are created. |
| JM-08 | `07_simulated_payment.jmx` | Controlled simulated payment | Smoke: 1; mutation: 1, 5, 10 | Pass if 0 JTL errors and stock/order changes are recorded. |
...
| Result generation | CSV, Markdown tables and SVG charts were generated. | Retained under `docs/testing/jmeter/results/phase6-summary/`. |
...
Project FYP Mall. 2026a. *D6 Implementation Document*. Internal project documentation. `report/D6_Implementation_20260707_v3.md` [Accessed 7 July 2026].

Project FYP Mall. 2026b. *D7 Testing Draft*. Internal project documentation. `report/D7_Testing_20260701_v1.md` [Accessed 7 July 2026].

Project FYP Mall. 2026c. *Verification Workflow*. Internal project documentation. `docs/verification/verification-workflow.md` [Accessed 7 July 2026].

Project FYP Mall. 2026d. *Phase 6 JMeter Performance Evaluation Report*. Internal project documentation. `docs/reports/phase-6-jmeter-performance-evaluation-report.md` [Accessed 7 July 2026].

Project FYP Mall. 2026e. *Phase 6 JMeter Execution Record*. Internal project documentation. `docs/records/phase-6-jmeter-execution-record-2026-06-16.md` [Accessed 7 July 2026].

Project FYP Mall. 2026f. *JMeter Summary Tables and Charts*. Internal project documentation. `docs/testing/jmeter/results/phase6-summary/summary-tables.md` [Accessed 7 July 2026].

---

## APPENDIX 4A: Source Artefact Locations
