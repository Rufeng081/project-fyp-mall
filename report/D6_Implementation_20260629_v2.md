...
| Table 1.2 | Implementation Traceability to D5 Objectives |
...
This document follows the D6 implementation guideline in `Panduan ProjekT_4086.docx`. The guideline states that D6 should explain the implementation of the system based on the requirement specification and design specification, but it does not need to describe every line of code. Therefore, this report selects only the code segments that are critical to system operation, technically relevant to the Network Technology project focus, or useful for explaining how the proposed design was realised.

**Table 1.1: D6 Guideline Coverage**

| Guideline requirement | How this D6 document addresses it |
|---|---|
| Explain the purpose and scope of the document | Section 1.1 and Section 1.2 define the purpose, scope, included components, and excluded commercial features. |
| Relate D6 to earlier and later project documents | Section 1.3 connects D6 to D5, D3, D4, D7, and D8. |
| Define important terms and abbreviations | Section 1.4 lists key technical terms used throughout the implementation report. |
| Explain the development process using the selected technologies | Section 2.1 explains the incremental implementation approach and technology selection. |
| Extract and explain critical code segments | Sections 2.4, 2.5, and 2.7 include selected frontend, backend, and deployment code listings. |
| Include important modules, components, libraries, and database | Sections 2.3, 2.5, 2.6, and 2.7 describe repository modules, backend modules, database schema, and deployment components. |
| Include frontend and backend interfaces | Sections 2.4 and 2.5 describe customer/admin UI implementation and REST API interfaces. |
| Explain unexpected problems and how they were handled | Section 3.2 lists major implementation problems, their impact, and their resolution evidence. |
| Provide references in UKM style | The References section uses an author-date web-reference style consistent with the D5 v3 report. |

**Table 1.2: Implementation Traceability to D5 Objectives**

| D5 objective | Implementation evidence in D6 |
|---|---|
| Objective 1: design and develop a cloud-based small e-commerce platform | Vue customer/admin pages, Spring Boot REST APIs, MySQL schema, Redis session state, email verification, cart, order, simulated payment, and order-history implementation. |
| Objective 2: implement and analyse network communication mechanisms | Axios HTTP client, Nginx public reverse proxy, Spring Boot internal service on port 9191, MySQL/Redis TCP communication, SMTP email communication, and public `/api/` path mapping. |
| Objective 3: evaluate network performance under different test parameters | Eight JMeter test plans, parameterised thread/ramp-up/loop values, response assertions, database backup discipline for mutation tests, and retained performance-result artefacts. |
...
The selected technology stack is shown in Table 2.1. Spring Boot was selected for the backend because it supports web, data access, mail, configuration, testing, and executable application packaging within one Java framework (Spring 2026). Vue 2 was used to build a component-based SPA, while Axios provides the client HTTP interface (Vue.js 2026; Axios 2026). MySQL provides the relational persistent store, Redis manages short-lived state, Nginx exposes the public application and reverse-proxies requests, and Apache JMeter executes parameterised HTTP load tests (Oracle 2026; Redis 2026; F5 NGINX 2026; Apache Software Foundation 2026).
...
The selected code listings in this report were chosen using three criteria from the D6 guideline. First, the code must be important to the operation of the system, such as session validation, order creation, or reverse-proxy routing. Second, the code should help the reader understand a technically significant part of the project, such as Redis TTL handling or public-to-internal request forwarding. Third, the code should demonstrate how the D5 proposal was implemented in practice. Full source code is not repeated in the main text because the D6 guideline states that only selected code needs to be explained; the complete source artefacts are listed in Appendix A and should be included in D8.
...
flowchart LR
    ...
    D -->|Session, verification-code and cache keys| F[(Redis)]
...
The deployment deliberately uses a single VM architecture because the FYP evaluates client-server-database communication under controlled load rather than a commercial distributed platform. Nginx is configured to serve the SPA and to forward `/api/` traffic to the backend service on `127.0.0.1:9191`, consistent with its reverse-proxy model (F5 NGINX 2026). This separation allows the public web server to be exposed on port 80 while the Spring Boot process remains behind the reverse proxy.
...
> **Manual figure note:** The screenshots above are inserted from existing project evidence. Before converting this Markdown report into the final Word/PDF thesis format, check image sharpness and scale. If the Markdown renderer does not preserve image sizing, manually insert the same files from `report/screenshot/` and `docs/assets/`.
...
The backend follows a controller-service-mapper-entity structure. Controllers expose HTTP endpoints and return a common `Result` envelope. Services contain business logic, transactions, and validation. Mapper interfaces and XML files implement SQL access through MyBatis and MyBatis-Plus (MyBatis 2026). Entities represent persistent data, while DTOs prevent unnecessary internal fields from being returned to the browser.
...
This dual mechanism was selected for the FYP system because JWT verifies token integrity while Redis enables server-side session expiry and immediate invalidation through key removal (Redis 2026). Public resources and pre-login authentication endpoints are excluded from the interceptor; protected business endpoints require a valid session. A second authority interceptor protects administrator operations.
...
The email-verification module was introduced to prevent direct unauthenticated registration and to support a usable forgotten-password flow. It uses Spring Boot Mail with Brevo SMTP credentials supplied at runtime (Brevo 2026). The service generates a cryptographically strong six-digit code, stores the code under a purpose-specific Redis key for five minutes, and writes a second cooldown key for 60 seconds. The purpose is either `register` or `reset`, so a code issued for one process cannot be used in the other.
...
The production implementation runs on an Ubuntu 22.04 Google Cloud VM (Google Cloud 2026). The application service is started by systemd under the `www-data` user. The service unit loads a protected environment file, starts the executable Spring Boot JAR, and restarts after failure. MySQL and Redis run as data services on the VM. Persistent media storage is configured through `MALL_UPLOAD_DIR` so uploaded resources are not tied to the lifecycle of the packaged JAR.
...
Apache JMeter 5.6.3 was used to implement reproducible HTTP test plans (Apache Software Foundation 2026). This section explains the implementation of the test artefacts because they are part of the developed Network Technology system. Detailed test planning, procedures, and result discussion belong to D7, but the D6 implementation report records how the testable artefacts were built.

Eight `.jmx` plans cover the homepage, product list, product detail, login, add-to-cart, place-order, simulated-payment, and order-history flows. Every plan is parameterised through `THREADS`, `RAMP_UP`, and `LOOPS`, enabling the same test design to be used for smoke and load execution.
...
> **Manual chart note:** If the final Word/PDF report cannot render SVG files, export these charts to PNG before insertion. A screenshot of one JMeter Thread Group, HTTP sampler, and response assertion may also be inserted in D7 when presenting detailed test procedures.
...
Oracle. 2026. *MySQL 8.0 Reference Manual*. Available at: https://dev.mysql.com/doc/refman/8.0/en/ [Accessed 29 June 2026].

Redis. 2026. *Redis Documentation*. Available at: https://redis.io/docs/ [Accessed 29 June 2026].
...
Vue.js. 2026. *Vue.js 2 Guide*. Available at: https://v2.vuejs.org/v2/guide/ [Accessed 29 June 2026].
...
Project FYP Mall. 2026b. *Database Design*. Internal project documentation. Available at: `docs/database/DATABASE_DESIGN.md` [Accessed 29 June 2026].

Project FYP Mall. 2026c. *Phase 3 Brevo Email Verification Report*. Internal project documentation. Available at: `docs/reports/phase-3-email-verification-report.md` [Accessed 29 June 2026].

Project FYP Mall. 2026d. *Phase 4 Cloud Deployment Report*. Internal project documentation. Available at: `docs/reports/phase-4-cloud-deployment-report.md` [Accessed 29 June 2026].

Project FYP Mall. 2026e. *Phase 6 JMeter Performance Evaluation Report*. Internal project documentation. Available at: `docs/reports/phase-6-jmeter-performance-evaluation-report.md` [Accessed 29 June 2026].
