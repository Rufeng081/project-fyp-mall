...
**System name:** Rufeng Mall
**Version:** Rewritten D5 version, 28 June 2026
...
| Figure 3.1 | Overall Cloud Deployment Architecture |
| Figure 3.2 | Network Request Flow |
| Figure 3.3 | Use Case Diagram |
| Figure 3.4 | Email Verification Sequence |
| Figure 3.5 | Order and Simulated Payment Sequence |
| Figure 3.6 | Database Entity Relationship Diagram |
| Figure 3.7 | Interface Screenshot Set |
| Figure 3.8 | JMeter Performance Charts |
...
**Objective 1:** To design and develop a cloud-based small e-commerce platform that supports core online shopping functions, including user authentication, product browsing, cart management, order placement, and simulated payment.

**Objective 2:** To implement and analyse the network communication mechanisms between the client browser, Nginx reverse proxy, Spring Boot application server, MySQL database, and Redis temporary-state service in a cloud VM environment.

**Objective 3:** To evaluate the deployed platform's network performance using Apache JMeter by measuring response time, throughput, and error rate under different concurrent user loads.
...
|---|---|---|
| D1 Project Planning | Define problem, objectives, scope, restrictions, and project direction | Project plan and approved project direction |
| D2 Literature Review | Review cloud web deployment, reverse proxy, REST, database, Redis, and performance testing | Literature foundation and research gap |
| D3 Requirement Analysis | Define user needs, functional requirements, non-functional requirements, OSI/protocol requirements, and system models | Requirements and methodology baseline |
| D4 Design Specification | Design architecture, topology, database, algorithms, interface, and performance-evaluation method | Design specification |
| D5 Proposal Report | Consolidate D1 to D4 into a proposal report aligned with Network Technology criteria | Current report |
| D6 Implementation | Implement and explain selected critical code, deployment, and verification evidence | Implementation document |
| D7 Testing | Execute and report functional, integration, deployment, and performance tests | Testing report |
| D8 Final Report | Combine updated chapters, implementation, testing, conclusion, limitations, and future work | Final thesis report |
...
The implemented system follows a client-server model. The Vue frontend runs in the browser after being served by Nginx. The frontend then uses Axios to send HTTP requests to backend REST endpoints. Requests and responses use JSON for structured data exchange.
...
From the OSI model perspective, REST and JSON are application-layer concerns, HTTP is the application-layer protocol used for web exchange, TCP provides reliable transport, and IPv4 routes traffic between the client and the cloud VM. MySQL and Redis also use TCP-based communication between the Spring Boot application and local data services.

## 2.4 Reverse Proxy and Nginx in Web Systems

A reverse proxy receives public requests and forwards selected traffic to internal services. Nginx is widely used for this role because it can serve static files, handle routing rules, forward headers, and proxy API traffic to an application server.

In Rufeng Mall, Nginx has two roles:
...
E-commerce applications require persistent data for users, products, carts, orders, and media. MySQL is used in this project as the relational database because the data has clear entities and relationships. The schema includes user accounts, addresses, product categories, products, variants, cart rows, order headers, order line items, carousel items, file metadata, and avatar metadata.

Redis is used for temporary state rather than durable business records. This separation is important. MySQL stores the long-term state of the e-commerce workflow, while Redis stores data that naturally expires or can be regenerated. Examples include login session records, email verification codes, resend cooldown keys, and selected cached product data.
...
Apache JMeter is a suitable tool for this project because it can generate HTTP requests, parameterise thread counts, apply assertions, and produce response-time, throughput, and error-rate data. In Rufeng Mall, JMeter plans are created for eight scenarios:
...
This chapter describes the methodology used to specify, model, design, and evaluate Rufeng Mall. It combines the D3 requirement-analysis perspective and the D4 design-specification perspective required for the D5 Proposal Report.
...
### 3.7.3 Network Communication Model
...
- `report/screenshot/main page.png` - homepage.
- `report/screenshot/products page.png` - product browsing.
- `report/screenshot/login page.png` - login interface.
- `report/screenshot/righter page.png` - registration interface.
- `report/screenshot/admin page.png` - administrator interface.
- `docs/assets/phase-2-order-history-verified.png` - verified order history.
- `docs/testing/jmeter/results/phase6-summary/charts/p90-response-time-ms.svg` - JMeter P90 chart.
- `docs/testing/jmeter/results/phase6-summary/charts/throughput-per-second.svg` - JMeter throughput chart.
...
This chapter has specified the user needs, requirements, network protocols, system models, architecture, database design, algorithms, interface evidence, and performance evaluation method for Rufeng Mall. The methodology is aligned with Network Technology because it treats the system as a deployed communication application and evaluates it through measurable HTTP performance indicators.
